...
> = {
  ...
  booking_completed: {
    label: 'Completed',
    accent: 'text-gray-500 dark:text-white/50 bg-gray-100 dark:bg-white/[0.06]',
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function NotificationItem({
  ...
  return (
    <div
      className={`group relative flex gap-3 px-4 py-3.5 border-b border-black/[0.05] dark:border-white/[0.05] transition-colors hover:bg-black/[0.03] dark:hover:bg-white/[0.03] ${
    ...
    >
      ...
      {/* Icon */}
      <div className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center mt-0.5 ${cfg.accent}`}>
        {cfg.icon}
      </div>

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-gray-800 dark:text-white/80 leading-none mb-1">
          {notification.title}
        </p>
        <p className="text-xs text-gray-500 dark:text-white/45 leading-snug">
      ...
      <button
        ...
        className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded text-gray-300 dark:text-white/25 hover:text-gray-600 dark:hover:text-white/60"
        aria-label="Dismiss"
      >
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        title="Notifications"
        className="relative p-2 rounded-lg text-gray-500 dark:text-white/40 hover:text-gray-800 dark:hover:text-white/80 hover:bg-black/[0.05] dark:hover:bg-white/[0.05] transition-colors"
      >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-gray-800 dark:text-white/80">Notifications</span>
            {unreadCount > 0 && (
        ...
        <div className="max-h-[360px] overflow-y-auto scroll-area-viewport">
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
              <div className="w-10 h-10 rounded-full bg-black/[0.04] dark:bg-white/[0.04] border border-black/[0.06] dark:border-white/[0.06] flex items-center justify-center mb-3 text-gray-300 dark:text-white/20">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                  <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" />
                  <path d="M13.73 21a2 2 0 01-3.46 0" />
                </svg>
              </div>
              <p className="text-xs font-medium text-gray-400 dark:text-white/30">No notifications yet</p>
        ...
        {/* Footer */}
        {notifications.length > 0 && (
          <div className="px-4 py-2.5 border-t border-black/[0.05] dark:border-white/[0.05]">
            <button
